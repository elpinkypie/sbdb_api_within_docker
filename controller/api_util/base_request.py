...
class Base(object):
    ...
    def send_request(self,
        ...
        # the condition block to check requests methods is standard and not shortened
        # in case if there will be other than GET requests types
...
class BaseAssertion:
    ...
    def verify_general_response_code_200(cls, res: Base.ResponseObject):
        cls.log_assert(res.status_code == 200,
                       "Assertion Failure, The status code is not 200, body: {}".format(res.text))

    @classmethod
    def verify_general_bad_request_code_400(cls, res: Base.ResponseObject):
        cls.log_assert(res.status_code != 400,
                       "Assertion Failure, The status code is not 400, body: {}".format(res.text))

    @classmethod
    def verify_general_not_allowed_code_405(cls, res: Base.ResponseObject):
